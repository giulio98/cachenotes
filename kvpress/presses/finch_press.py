...
from kvpress.presses.utils import compute_window_attention_triton
    ...
    def score(self, module, hidden_states, keys, values, attentions, kwargs):
        ...
        if attentions is not None:
            ...
            if self.normalize_scores:
                counts = torch.arange(q_len - self.window_size, q_len, device=attn_weights.device,
                        dtype=attn_weights.dtype).view(1, 1, -1, 1)
                attn_weights.mul_(counts)  # in-place
            ...
            if self.chunk_length is not None:
                # print("Using chunked attention scoring with chunk length", self.chunk_length)
                
                scores = SnapKVPress.compute_chunked_window_attention(
                    module, hidden_states, keys, self.window_size, kwargs["position_embeddings"], self.chunk_length, self.normalize_scores
                )
                """
                scores = compute_window_attention_triton(
                    module,
                    hidden_states,
                    keys,
                    self.window_size,
        ...
        if self.use_vnorm:
            scores = (scores) * values[..., :-self.window_size, :].norm(dim=-1)
            scores[..., :self.n_sink] = scores.max().item()
