...
class KVPressTextGenerationPipeline(Pipeline):
    ...
    def _sanitize_parameters(
        ...
        prompt_cheatsheet: Optional[str] = None,
        ...
        **kwargs : dict
        ...
        preprocess_kwargs = {
            ...
            "prompt_cheatsheet": prompt_cheatsheet,
    ...
    def preprocess(
        ...
        if self.tokenizer.chat_template is None:
            ...
            if cheatsheet is not None and prompt_cheatsheet is not None:
                context = context + prompt_cheatsheet + "\n" + cheatsheet
            ...
            if cheatsheet is not None and prompt_cheatsheet is not None:
                context = self.tokenizer.apply_chat_template(
                    [
                        {"role": "user", "content": context + prompt_cheatsheet},
                        {"role": "assistant", "content": cheatsheet + separator},
                    ],
        ...
        question_suffix_ids = self.tokenizer.encode(question_suffix, return_tensors="pt", add_special_tokens=False)
        ...
        if context_ids.shape[1] > max_context_length:
            ...
            head_len = max_context_length // 2                  # first half
            tail_len = max_context_length - head_len            # remainder (handles odd N)
            head = context_ids[:, :head_len]
            tail = context_ids[:, -tail_len:]
            context_ids = torch.cat([head, tail], dim=1)

        return {"context_ids": context_ids, "questions_ids": question_ids, "questions_suffix_ids": question_suffix_ids}
    ...
    def _forward(
        ...
        if press is not None:
            requested_compression_ratio = press.compression_ratio

        if isinstance(press, FinchPress):
            # --- extract original context (remove the delimiter token) ---
            first_split = int(torch.nonzero(context_ids[0] == press.delimiter_token_id)[0].item())
            context_length = first_split
            if max_capacity_context is not None:
                if first_split <= max_capacity_context:
                    apply_compression = False
                    context_ids = context_ids[:, :first_split]
            else:
                desired_keep = int(first_split * (1 - press.compression_ratio)) + 1 # +1 for the delimiter token
                press.compression_ratio = 1.0 - (desired_keep / float(context_ids.shape[1]))
            
        if max_capacity_context is not None:
            L = context_ids.shape[1]
            C = max_capacity_context
            if isinstance(press, FinchPress) and apply_compression:
                C = C + 1 # account for the delimiter token
            compression_ratio = max(0.0, 1.0 - C / float(L))
            press.compression_ratio = compression_ratio
        if press is not None:
            print(f"Using compression ratio: {press.compression_ratio}")
        ...
        if apply_compression:
            ...
            # No compression, standard pre-filling
            self.model.model(
                input_ids=context_ids,
                past_key_values=cache,
            )
        if press is not None:
            press.compression_ratio = requested_compression_ratio
        print(f"Context Length: {context_length}")
        print(f"Compressed Context Length: {cache.get_seq_length()}")
