...
class EvaluationConfig:
    ...
    compress_task_cheatsheet: bool = False
    ...
    def get_results_dir(self, output_dir: Path) -> Path:
        ...
        if self.max_capacity_context is None:
            components = [
                self.dataset,
...
class EvaluationRunner:
    ...
    def _load_and_prepare_dataset(self):
        ...
        if "rag" in dataset_name:
            assert self.config.max_context_length is not None, "max_context_length must be set for rag"
            dataset = DATASET_REGISTRY[dataset_name].format(max_context_length=self.config.max_context_length)
        else:
            dataset = DATASET_REGISTRY[dataset_name]
        logger.info(f"Loading dataset: {dataset} (data_dir: {data_dir})")
        df = load_dataset(dataset, data_dir, split="test", download_mode='force_redownload').to_pandas()
        ...
        if self.config.compress_questions:
            ...
            if data_dir == "lcc" or data_dir == "repobench-p":
                df["context"] = df["context"] + "\nPlease complete the given code snippet\n"
            df["context"] = df["context"] + df["question"]  # type: ignore[index]
            df["question"] = df["question"]  # type: ignore[index]
        
        if self.config.compress_task_cheatsheet:
            logger.info("Compressing task cheatsheet into context.")
            df["question"] = df["question"]  # type: ignore[index]
    ...
    def _run_inference(self):
        ...
        for context, df_group in tqdm(df_context_grouped, total=self.df["context"].nunique(), desc="Running Inference"):  # type: ignore[union-attr]
            ...
            # check if df has field "prompt_cheatsheet" and if so, add it to the context
            if self.config.compress_task_cheatsheet and ("prompt_cheatsheet" in df_group.columns and "cheatsheet" in df_group.columns):
                prompt_cheatsheet = df_group["prompt_cheatsheet"].iloc[0]
                cheatsheet = df_group["cheatsheet"].iloc[0]
            else:
                prompt_cheatsheet = None
                cheatsheet = None
            ...
            output = self.pipeline(  # type: ignore[misc]
                ...
                max_capacity_context=self.config.max_capacity_context,
            )
