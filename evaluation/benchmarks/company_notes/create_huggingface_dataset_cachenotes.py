...
def parse_args():
    ...
    p.add_argument("--repo_id", type=str, default="giulio98/company-notes")
    p.add_argument("--max_model_len", type=int, default=None)
    p.add_argument("--rope_scaling_json", type=str, default=None)
...
cheat_sheet_prompt = "Before I give you the question, imagine you are a student memorizing this material according to the task that you will have to perform. Repeat the context concisely yet comprehensively to aid memorization, preserving all critical details and creating a cheat sheet. Once done, I will give you the question.\n\n"
...
def main():
    ...
    if args.max_model_len and args.rope_scaling_json:
        rope = json.loads(args.rope_scaling_json)

        # Accept either {"type":"yarn", ...} (HF-style) or {"rope_type":"yarn", ...} (vLLM-style)
    ...
    hf_ds = load_dataset("giulio98/company-notes", split="test")
    ...
    prompts = []
    for i in range(n):
        msgs = [{"role": "user", "content": rows["context"][i] + rows["prompt_cheatsheet"][i]}]
    ...
    final_ds.push_to_hub(args.repo_id, split="test")
